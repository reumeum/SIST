...
public class OrderDAO {
	...
	public void insertOrder(OrderVO order, List<OrderDetailVO> orderDetailList) throws Exception {
		...
		try {
			...
			// 주문정보 처리
			sql = "INSERT INTO zorder (order_num,order_total," + "payment,receive_name,receive_post,receive_address1,"
					+ "receive_address2,receive_phone,notice,mem_num) " + "VALUES (?,?,?,?,?,?,?,?,?,?)";
	...
	public int getOrderCountByMem_num(String keyfield, String keyword, int mem_num) throws Exception {
		...
		try {
			...
			sql = "SELECT COUNT(*) FROM zorder JOIN (SELECT order_num,"
					+ "LISTAGG(item_name,',') WITHIN GROUP (ORDER BY item_name) "
					+ "item_name FROM zorder_detail GROUP BY order_num) " + "USING (order_num) WHERE mem_num=? "
					+ sub_sql;
			// PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			// ?에 데이터 바인딩
	...
	public List<OrderVO> getListOrderByMem_num(int start, int end, String keyfield, String keyword, int mem_num)
		...
		try {
			...
			// SQL문 작성
			sql = "SELECT * FROM (SELECT a.*, rownum rnum FROM (" + "SELECT * FROM zorder JOIN (SELECT order_num,"
					+ "LISTAGG(item_name,',') WITHIN GROUP (ORDER BY item_name) "
					+ "item_name FROM zorder_detail GROUP BY order_num) " + "USING (order_num) WHERE mem_num=? "
